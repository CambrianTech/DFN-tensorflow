...
def model_fn(features, labels, mode, params, config):
	...
	pix_semantic = features["pix_semantic"]
	inputs = tf.concat((input_image, pix_semantic), axis=-1)
...
def get_input_fn(input_dir, pix_dir, output_dir, shuffle, batch_size, epochs, crop_size, classes, augment):
	def parse_image(input_file_name, pix_file_name, output_file_name):
		...
		pix_semantic = _parse(pix_file_name, crop_fraction, channels=1)
		...
		return {"image": image, "pix_semantic": pix_semantic}, output

	def input_fn():
		file_seed = np.random.randint(10000000)
		input_files = tf.data.Dataset.list_files(input_dir, seed=file_seed)
		pix_files = tf.data.Dataset.list_files(pix_dir, seed=file_seed)
		output_files = tf.data.Dataset.list_files(output_dir, seed=file_seed)

		dataset = tf.data.Dataset.zip((input_files, pix_files, output_files))
		if shuffle:
...
def get_serving_input_receiver_fn(crop_size):
	def serving_input_receiver_fn():
		inputs = {
			...
			"pix_semantic": tf.placeholder(tf.float32, [None, crop_size, crop_size, 1]),
		}
